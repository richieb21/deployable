...
class GitHubAPIClient:
    ...
    def check_repository_exists(self, owner: str, repo: str) -> bool:
        ...
        try:
            ...
            if not exists:
                logger.error(
                    f"Repository check failed with status {response.status_code}: {response.text}"
                )
            return exists
    ...
    def check_deployable_file_exists(self, owner: str, repo: str) -> bool:
        ...
        try:
            ...
            if not exists:
                logger.debug(
                    f".deployable file check returned status {response.status_code}"
                )
            return exists
        except Exception as e:
    ...
    def check_issues_enabled(self, owner: str, repo: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Issues check failed with status {response.status_code}: {response.text}"
            )
            return False
        except Exception as e:
    ...
    def create_issue(
        ...
        if not self.check_repository_exists(owner, repo):
            ...
            raise ValueError(
                f"Repository '{owner}/{repo}' does not exist or is not accessible with current token"
            )

        # Then check if .deployable file exists
        if not self.check_deployable_file_exists(owner, repo):
            logger.error(
                f"Repository '{owner}/{repo}' does not have a .deployable file"
            )
            raise ValueError(
                f"Repository '{owner}/{repo}' does not have a .deployable file in the root directory"
            )
        ...
        if (
            ...
            if response.status_code == 403:
                raise PermissionError(
                    f"Permission denied: {error_message}. Check if your token has the 'repo' scope."
                )
            elif response.status_code == 404:
                raise ValueError(
                    f"Not found: {error_message}. Repository might not exist or issues might be disabled."
                )
            else:
                raise Exception(
    ...
    def validate_token(self) -> Dict[str, Any]:
        ...
        if not self.token:
            ...
            raise ValueError(
                "GitHub token is not set. Please set the GITHUB_PERSONAL_ACCESS_TOKEN environment variable."
            )
        ...
        if response.status_code != 200:
            ...
            raise ValueError(
                "Invalid GitHub token or token has insufficient permissions"
            )
