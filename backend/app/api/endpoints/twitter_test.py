...
async def test_twitter_credentials():
    ...
    if results.get("oauth1", {}).get("success", False):
        try:
            response = oauth1_client.create_tweet(text="Test tweet from API credential check")
            results["tweet_creation"] = {
                "success": True,
                "tweet_id": response.data["id"]
            }
            logger.info(f"Successfully created test tweet with ID: {response.data['id']}")
        except Exception as e:
            results["tweet_creation"] = {
                "success": False,
                "error": str(e)
