...
async def create_issue(request: CreateIssueRequest):
    ...
    try:
        ...
        if not github_api_client.check_repository_exists(request.owner, request.repo):
            logger.error(
                f"Repository '{request.owner}/{request.repo}' does not exist or is not accessible"
            )
            raise HTTPException(
                status_code=404,
        ...
        logger.info(
            f"Checking if .deployable file exists in {request.owner}/{request.repo}"
        )
        if not github_api_client.check_deployable_file_exists(
            request.owner, request.repo
        ):
            logger.error(
                f"Repository '{request.owner}/{request.repo}' does not have a .deployable file"
            )
            raise HTTPException(
                status_code=403,
        ...
        logger.info(
            f"Checking if issues are enabled for {request.owner}/{request.repo}"
        )
        if not github_api_client.check_issues_enabled(request.owner, request.repo):
            logger.error(
                f"Issues are disabled for repository '{request.owner}/{request.repo}'"
            )
            raise HTTPException(
                status_code=400,
